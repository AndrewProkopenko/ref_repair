...
$('.ref-price_btn button').click(function() {
    $('.ref-price_col-small:nth-child(n+6)').show();
    $('.ref-price_btn button').remove();
    return false;
});
...
// set city function 
$('#yourcity').change( setCity )

$(window).on('load', setCity)

function setCity() {  
    if( window.innerWidth > 992) { 
        let selectedOption = $('#yourcity option[selected]')[0]  
        
        let url_one = 'assets/img/screen-one/' + $(selectedOption).attr('data-screen-one')
        let url_two = 'assets/img/screen-three/' + $(selectedOption).attr('data-screen-two')
    
        $('.js_screen_one_bg').css('background-image' , 'url(' + url_one + ')' ) 
        $('.js_screen_two_bg').css('background-image' , 'url(' + url_two + ')' )
    }
